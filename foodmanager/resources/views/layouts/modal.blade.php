        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="food-form" action="{{ route('calendar') }}" method="post">
                    ...
                    <div class="form-group">
                        <label for="food-name"><i class="fas fa-carrot mr-2"></i>食物名稱:</label>
                        <input type="text" class="form-control border-success" id="food-name" name="food_name" required>
                    </div>
                    <div class="form-group">
                        <label for="category"><i class="fas fa-th-large mr-2"></i>類別:</label>
                        <div class="custom-dropdown" id="category-dropdown">
                                ...
                                <div class="dropdown w-100">
                                    <button class="btn btn-outline-success dropdown-toggle w-100" type="button"
                                        ...
                                        <span id="selected-category">
                                            <img src="@if($getFoodCategory->first()->img_url){{ asset('img/'.$getFoodCategory->first()->img_url) }}@else{{ asset('img/01.jpg') }}@endif"
                                                 style="width: 50px;" alt="{{ $getFoodCategory->first()->name }}" class="dropdown-icon">
                                            {{ $getFoodCategory->first()->name }}
                                        </span>
                                    </button>
                                    <div class="dropdown-menu w-100" aria-labelledby="dropdownMenuButton">
                                        @foreach($getFoodCategory as $item)
                                            <div class="dropdown-item category-item" data-value="{{ $item->name }}" data-name="{{ $item->id }}"
                                                 data-img="@if($item->img_url){{ asset('img/'.$item->img_url) }}@else{{ asset('img/01.jpg') }}@endif">
                                                <img src="@if($item->img_url){{ asset('img/'.$item->img_url) }}@else{{ asset('img/01.jpg') }}@endif"
                                                     style="width: 50px;" alt="{{ $item->name }}" class="dropdown-icon img-thumbnail"> {{ $item->name }}
                                            </div>
                                        @endforeach
                                    </div>
                                </div>
                    ...
                    <div class="form-group">
                        <label for="storage-location"><i class="fas fa-map-marker-alt mr-2"></i>存放位置:</label>
                        <input type="text" class="form-control border-success" id="storage-location" name="storage_location" placeholder="例如: 冰箱、櫥櫃">
                    </div>
                    <div class="form-group">
                        <label for="purchase-date"><i class="fas fa-calendar-alt mr-2"></i>購買日期:</label>
                        <input type="date" class="form-control border-success" id="purchase-date" name="purchase_date" value="{{ date('Y-m-d') }}" required>
                    </div>
                    <div class="form-group">
                        <label for="quantity"><i class="fas fa-sort-numeric-up mr-2"></i>數量:</label>
                        <div class="input-group">
                    ...
                    <div class="form-group">
                        <label for="expiry-date"><i class="fas fa-calendar-check mr-2"></i>有效期限:</label>
                        <input type="date" class="form-control border-success" id="expiry-date" name="expiry_date" value="{{ date('Y-m-d', strtotime('+3 days')) }}" required>
                    </div>
                    <div class="form-group">
                        <label for="food-status"><i class="fas fa-info-circle mr-2"></i>食物狀態:</label>
                        <select class="form-control border-success" id="food-status" name="food_status">
